...
public class Map {
  ...
  public City lookup(String cityName) {
    ...
    City added = new City(cityName,id++);
    for (int i = 0; i < cities[index].length; i++) {
  ...
  public Path dijk(City source, City destination){
    Path shortest = dijk(source, destination);
    return shortest;
  }
    ...
    public static void main(String[] args) {
        ...
        Map cityMap = new Map("europe.csv");
            ...
            if (bucket != null) { // Check if the bucket is not null (there are cities in this index)
                for (int j = 0; j < bucket.length; j++) {
                    ...
                    if (city != null) { // Check if the city is not null
                        ...
                        for (int k = 0; k < city.getSize(); k++) {
                            Connection conn = city.getConnection(k);
